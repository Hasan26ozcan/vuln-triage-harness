...
├── data/                   # Auxiliary data (e.g. cve_cwe_mapping.json)
...
├── docker-compose.infra.yml  # Infrastructure services: Postgres + MinIO + Redis
...
├── data/                   # Auxiliary data (cve_cwe_mapping.json tracked;
│                           # data/raw, data/processed, data/cvefixes_db gitignored)
├── .env.example            # Environment variable template (committed)
└── LICENSE
```

> **Note:** Several directories are gitignored (`.venv/`, `tools/llama-cpp/`,
> `tools/llama-cpp.zip`, `output/`, `data/raw/`, `data/processed/`,
> `data/cvefixes_db/`, `*.jsonl`, `*.gguf`, `*.safetensors`, `wandb/`, `.env`).
...
# 2. Bring up Postgres + MinIO + Redis (infrastructure services)
...
# Note: the GPTQ output directory `output/stage8/gptq_bits4/` contains
# `config.json` + `quantize_config.json` metadata from the quantization
# run; the resulting safetensors checkpoint is produced by the standalone
# `scripts/run_stage8_real.py` script (see Stage 8 Real-Run Results below).
```
...
(~5.8 GB F32) runs entirely on the GPU.
...
#    The default path is ./output/quantized/model.gguf (a symlink
#    to output/stage9/tuned_model.gguf). Override with GGUF_MODEL_PATH.
#    F32:   output/stage9/tuned_model.gguf  (~5.8 GB, full precision)
#    GPTQ:  output/stage8/gptq_bits4/       (config.json + quantize_config.json
#           from Stage 8 real run; safetensors checkpoint produced by
#           scripts/run_stage8_real.py on a GPU machine)
# The F32 GGUF model is produced by:
#   python scripts/merge_lora_for_export.py  (merge LoRA → HF dir)
#   python scripts/convert_to_gguf.py        (HF dir → GGUF F32)
# The model is then symlinked to ./output/quantized/model.gguf for serving.
...
> **Note:** The GPU serving container uses `llama-cpp-python` compiled
> with CUDA (`GGML_CUDA=on`) inside the container. The `transformers`
> backend (shown in the real run below) is an alternative path that
> works on the host directly without a Docker build.
...
# 6. Or use the transformers backend directly (no Docker needed)
python -m app.evaluation.cli stage9 serve \
  --backend transformers \
  --model-path ./output/stage9/tuned_merged
```
...
The GGUF checkpoint (`output/stage9/tuned_model.gguf`, ~5.8 GB F32) is the
...
| GGUF checkpoint | `output/stage9/tuned_model.gguf` (~5.8 GB, F32) |
...
pytest tests/integration/test_stage7_script.py       # Stage 7 local runner
...
| `make infra` | Start infrastructure services (Postgres + MinIO + Redis): `docker compose -f docker-compose.infra.yml up -d` |
| `make up` | Start all services (infra + GPU serving): runs `make infra` then `docker compose --profile gpu up serving-gpu -d` |
| `make down` | Stop all services: `docker compose -f docker-compose.infra.yml down && docker compose -f docker-compose.yml --profile gpu down` |
...
1. **Copy environment variables:** `cp .env.example .env` and fill in your
   values (Postgres, MinIO, HuggingFace, NVD API, OpenAI keys).
2. **Start infrastructure:** `make infra` — brings up Postgres, MinIO,
   and Redis via `docker-compose.infra.yml`.
3. **Install dependencies:** `pip install -e ".[dev,data,ml]"`
4. **Run tests:** `pytest tests/ -v` (all should pass without GPU/network)
5. **Check lint:** `ruff check .`
6. **Check types:** `mypy app --config-file pyproject.toml` (or `make typecheck`)
7. **Check security:** `bandit -r app -q`
8. **Scan for vulns:** `pip-audit` (or `trivy fs --skip-dirs .venv,output --severity CRITICAL,HIGH .`)
9. **Make your changes** — follow the lazy-import pattern for ML deps,
   implement `Protocol` interfaces for injectable backends, and add unit
   tests that use mock backends.
10. **Run tests again** to ensure nothing regresses.

> **Note:** The `CWE_SCOPE`, `TRAINING_METHODS`, `BASE_MODEL`, and
...
> `eval/gold_set/*.jsonl` which are tracked), `*.gguf`, `*.safetensors`,
> and `wandb/` are gitignored — see `.gitignore` for the full list.
> Infrastructure services (Postgres, MinIO, Redis) are
> defined in `docker-compose.infra.yml`; GPU serving is in `docker-compose.yml`.
