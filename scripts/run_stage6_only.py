...
if TYPE_CHECKING:
    ...
    from app.schemas.prediction_eval import EvalMetrics, EvalReport
...
def _parse_args() -> argparse.Namespace:
    """Parse command-line arguments for the Stage 6 script."""
...
def _wrap_progress_logging(evaluator: LlmJudge) -> None:
    """Wrap *evaluator*'s ``evaluate_all`` with progress logging."""
    ...
    def _progress_eval_all(samples, predictions):
        ...
        for i, sample in enumerate(samples):
            ...
            if pred is None:
                continue
            results.append(evaluator.evaluate(sample, pred))
            if (i + 1) % 5 == 0 or i + 1 == len(samples):
                print(f"  Judge: {i + 1}/{len(samples)} samples done", flush=True)
        return results

    evaluator.evaluate_all = _progress_eval_all


def _setup_tier4_evaluator(args: argparse.Namespace) -> LlmJudge | None:
    """Load the trained model and create a Tier 4 LLM judge evaluator.

    Returns ``None`` when ``--skip-tier4`` was passed.
    """
    if args.skip_tier4:
        return None
    ...
    _wrap_progress_logging(evaluator)
    return evaluator


def _print_metrics(m: EvalMetrics) -> None:
    """Print Stage 6 evaluation metrics."""
    print("\nStage 6 Metrics:")
...
def _load_training_result() -> dict:
    """Load the training result JSON from the canonical Stage 5 output location.

    The checkpoint path is intentionally NOT accepted as a parameter — the
    training result is always read from the fixed ``output/stage5/training_result.json``
    location to prevent path traversal via user-controlled CLI arguments.
    """
    tr_path = validate_path("output/stage5/training_result.json", allow_temp=True)
    if not tr_path.exists():
        raise FileNotFoundError(
            f"Training result not found at {tr_path}. "
            "Run Stage 5 training first to generate it."
        )
    return json.loads(safe_read_text(tr_path, allow_temp=True))


def _load_stage4_metrics() -> dict:
    """Load Stage 4 metrics, returning an empty dict when the file is absent."""
    stage4_metrics_path = validate_path("output/stage4/metrics.json", allow_temp=True)
    if not stage4_metrics_path.exists():
        return {}
    return json.loads(safe_read_text(stage4_metrics_path, allow_temp=True))
...
def _save_combined_output(
    ...
    """Save the combined Stage 4 + Stage 6 output."""
    output = {
...
def _print_summary(args: argparse.Namespace, stage4_metrics: dict, m: EvalMetrics) -> None:
...
def main():
    """Run Stage 6 evaluation using saved Stage 4 predictions."""
    args = _parse_args()
    ...
    tier4_evaluator = _setup_tier4_evaluator(args)
    ...
    training_result = _load_training_result()
    stage4_metrics = _load_stage4_metrics()
    _save_combined_output(args, eval_report, stage4_metrics, training_result, m)

    # Print summary
    _print_summary(args, stage4_metrics, m)
